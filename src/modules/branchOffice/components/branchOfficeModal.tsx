                    ...
                    <Tabs defaultActiveKey='1' appearance='pills' >
                        <Tabs.Tab eventKey="1" title="Información General">
                            <Form.Group style={{ flex:1 }}>
                                    ...
                                    <Form.Control value={formData.name} name="name" placeholder="Ingrese el nombre de la sucursal *" onChange={(value) => updateField('name', value)} />
                                </Form.Group>
                                <div style={{ display: 'flex', justifyContent:'space-between', gap:20 }}>
                                    <Form.Group style={{flex:1}}>
                                        <Form.ControlLabel><strong>Estado</strong></Form.ControlLabel>
                                        <SelectPicker block value={formData.status} data={filterStatusOptions} searchable={false} onChange={(value) => updateField('status', value)} placeholder="Seleccione el estado *"/>
                                    </Form.Group>
                                    <Form.Group style={{flex:1}}>
                                        <Form.ControlLabel><strong>Responsable</strong></Form.ControlLabel>
                                        <SelectPicker block value={formData.userId} data={handleUsersOptions(users)} loading={loading} onChange={(value) => updateField('userId', value)} searchable={false} placeholder="Seleccione el nombre del responsable *"/>
                                    </Form.Group>
                                </div>
                                <Form.Group>
                                    <Form.ControlLabel><strong>Dirección de la Sucursal</strong></Form.ControlLabel>
                                    <Input placeholder="Ingrese la dirección de la sucursal *" value={formData.address} name="address" as={'textarea'} rows={5} onChange={(value) => updateField('address', value)} />
                                </Form.Group>
